...
const __nuxt_component_0 = Object.assign(_sfc_main$1, { __name: "ScraperLogConsole" });
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const canCancelJob = computed(() => {
      return fetchStatus.value && (fetchStatus.value.status === "pending" || fetchStatus.value.status === "running");
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b, _c;
      const _component_ScraperLogConsole = __nuxt_component_0;
      ...
      if (unref(isFetching) || unref(fetchStatus)) {
        _push(`<div class="card p-4 mb-6"><div class="flex justify-between items-center mb-2"><span class="font-medium text-slate-100">${ssrInterpolate(unref(fetchStatusText))}</span><div class="flex items-center gap-3"><span class="text-sm text-slate-400">${ssrInterpolate(unref(fetchProgress))}%</span>`);
        if (unref(canCancelJob)) {
          _push(`<button${ssrIncludeBooleanAttr(unref(isCancelling)) ? " disabled" : ""} class="btn btn-danger text-xs py-1 px-3">${ssrInterpolate(unref(isCancelling) ? "Avbryter..." : "Avbryt")}</button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div><div class="w-full bg-slate-700 rounded-full h-2.5"><div class="bg-primary-500 h-2.5 rounded-full transition-all duration-300" style="${ssrRenderStyle({ width: unref(fetchProgress) + "%" })}"></div></div>`);
...
//# sourceMappingURL=scraping-DkaTqVDn.mjs.map
