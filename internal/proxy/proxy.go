...
func NewBrightData(apiKey, country string) (*BrightData, error) {
	if apiKey == "" {
		return nil, fmt.Errorf("BrightData API key is required")
	}
	return &BrightData{apiKey: apiKey, country: country}, nil
}
...
func (p *BrightData) GetProxyURL() (string, error) {
	...
	return fmt.Sprintf("http://%s-session-%s:zproxy%slum-super-proxy:%s@zproxy.lum-super-proxy.io:22225",
		country, p.apiKey, p.apiKey, p.apiKey), nil
}
